---
title: Production Orders (Ordini di Produzione)
sidebar_position: 3
---
...
## Production Order Header (Testata ordine di produzione)
...
> **Generate Bill of Materials (Genera distinta base)**: allows generating the bill of materials based on the data entered in the production order or updating existing data with those entered in the order;  
> **Generate Work Cycle (Generazione ciclo di lavoro)**: allows creating the work cycle of the article based on the data entered in the order, or updating existing data with those entered in the order;  
...
**Required Quantity (Q.tà impiego)**: represents the unit quantity necessary for this article (as per the BOM) which can be modified;  
**Total Quantity (Q.tà totale)**: represents the quantity obtained by multiplying the *Usage Quantity (Q.tà d'impiego)* by the *Quantity to Produce (Q.tà da produrre)*;  
**Alternative Quantity (Q.tà alternativa)**: represents the total quantity expressed in the alternative unit of measure;  
...
**Available Quantity (Q.tà disp)**: in this field, the available quantity of the article as of the usage date appears;  
**To be declared on mobile (Da dichiarare su mobile)**: if enabled, it will be declared on mobile.
...
**Phase Code/Phase/Subphase (Codice fase/Fase/Sotto Fase)**: with a double click, the relevant help for processing phases opens from which the corresponding phase and subphase can be selected;  
**Ctrl. qual.**: this flag indicates whether the material should be subjected to quality control before its use;  
**Production Phase (Fase prod)**: if the flag is checked, it identifies that this phase is a production phase, and therefore, must be reported;  
...
**Expected Start/End Date (Data inizio/fine prevista)**: these are the dates of the beginning and end of the relevant phase; changing the expected start and end dates will automatically modify these as well. They are calculated based on the times entered in the processing phases, and specifically based on the greater of the worker time and total machine time for the selected phase;  
...
**Quantity of Pieces per Phase (Qtà pezzi per fase)**: indicates the number of pieces per phase;  
...
**Subcontractor Account/Subcontractor Description (Conto terzista/descrizione terzista)**: with a double click on the box, the help opens to select the account and subaccount of the relevant subcontractor. This *box is active only* if the phase is designated as *External* phase. Note that the *subcontractor* is also retrieved from the processing phase entered in the routing of the article;  
**Work Order Created (Ord. CL creato)**: if checked, indicates that the relevant work order has already been created;  
...
**Worker Cost (Costo operaio)**: is valued from when I start making production reports;    
...
### Phases - Properties (Fasi - Proprietà)
...
**Overlap (Sovrapp)**: through this combo, I can insert the type of any overlap between phases. We can have *Total* overlap (in this case, the phase in question is completely overlapped with the next indicated phase), *By pieces* (in this case, it is necessary to indicate after how many pieces produced by this phase the next phase will start), *By time* (in this case, it is necessary to indicate after how many minutes from when the phase in question started the next phase will start);  
...
**Overlapping Related to Phase/Subphase (Sovrapposiz. riferita alla fase/sottofase)**: here the code of the phase and subphase that have an overlap with the phase in question is indicated. Usually, the next phase is indicated, but again, this is just a convention; 
...
### Phases - Extra Data (Fasi - Extra Data)
...
**Number of Machines (Num. macchine)**: indicates the number of machines used for the setup;  
**Number of Workers (Num. operai)**: indicates the number of workers used for the setup.
...
## Operating Instructions (Istruzioni opertative)
...
For details on the common functionality of forms, please refer to the link [Common functionalities, buttons, and fields](/docs/guide/common).