---
title: Returns and Credit Note Management (Gestione resi e note di credito)
sidebar_position: 3
---
...
**Return Registration (Registrazione del Reso)**: When a customer returns a product, the process begins with the creation of a return document in the management system. This document can be created manually (by entering a delivery note with Nature *Return* or an invoice with Nature *Credit Note (Nota di credito)*), or automatically with the **Transfer** procedure available in the [Search](/docs/sales/sales-delivery-notes/insert-delivery-notes/search-sales-dn) of the documents. In the case that a return delivery note is created, the credit note can be automatically created from it using the appropriate procedures.

**Stock Update (Aggiornamento delle Scorte)**: Once the cancellation document has been processed in the warehouse, the system automatically updates the warehouse stock to reflect the return of the goods, marking the returned products as available.

**Credit Note Details (Dettagli della Nota di Credito)**: The credit note includes details such as the credited amount and can be reviewed before being sent to the customer. Upon saving a credit note, all expense types are entered as negative, except for stamp duties.

**Accounting Update (Aggiornamento della Contabilità)**: Once accounted for, the credit note updates the accounting records, reducing the amount owed by the customer; this may reflect in a decrease of the open balance or in a refund, depending on the agreements with the customer.
...
### Management of Mixed Signs (Gestione segni misti)

The ability to account for the document with mixed signs must be specifically enabled through an SQL script.

:::note Technical Note for Activation (Nota tecnica per attivazione)
