---
title: Other specific configurations for electronic invoicing 
sidebar_position: 4
---
...
Regarding electronic invoicing, it is required for the checks performed by ES that the document sign be positive; the XML file generation parser will thus reverse the sign again (when the document type is TD04).
...
### Item coding for customer  

The item code tag can be repeated multiple times, generating a double coding by changing the code in the tag 2.2.1.3.1 

```xml
    <CodiceTipo> . 
```
The default proposed Type Code (in sales invoices) refers to the item code present in Fluentis (from the item registries of the company in use) which can be reported in two different ways:
...
```xml
    <CodiceArticolo> 
    <CodiceTipo>Supplier Item code</CodiceTipo>    
...
It is possible to support the coding in the **[Barcode Types](/docs/configurations/tables/general-settings/barcode-types)** table.
...
Then in **[Item registry](/docs/erp-home/registers/items/create-new-item)**, the value for each type of code is indicated:
...
Regarding the possibility of a double coding in the invoice, by adding that of the customer document holder, the customer item code, if entered in the Customers tab of the item registry, related to the customer of the invoice, can be expressed in two different ways:

- with the CodiceTipo tag having the wording “Customer Item code” 
...
- with the CodiceTipo tag equal to the customer item code (of the pair of fields *supplier/customer item code type*) present in customer registry in the *fiscal information* tab.
...
Where CODCLI is the value entered in the *customer item code type* field in customer registry.

:::tip Insight
If you want to display the Fluentis item code differently from the standard, you must customize in ARM the transformation of FSItem called FullTextDescriptionXMLFE. If you do not want to insert anything in the CodiceArticolo tag for the “supplier item code" CodiceTipo, in the transformation, in the grid, corresponding to the string “Engine Parameters,” nothing should be inserted. To exclude the class code in individual lines of the document, it is necessary to customize in Arm the FullTextDescription transformation of the FSItem object by modifying the text in the “Default” line's EngineParameters section from “ItemClassCode +”-“+ Code” to “Code.” 
:::
...
2.2.1.16.2 ReferenceText records the identifying and progressive reference by reading from the **[register of intent declarations](/docs/finance-area/declarations/declarations/intent-declaration)**
...
To manage annotations that in the prints of old invoices were firmly entered in the report (example “Contribution Conai fulfilled where due”), new notes must be entered in the Encoded Notes table (Utility->Manage encoded notes). This type of annotations are reported in the tag Reason of the XML file. 
...
On each item line, therefore, the user can freely (manually) add these additional data; as a result, the item line will have, (for example, if the extra data is of string type), this additional section: 
...
Some public entities require an ‘administration reference’ for each item line (and not in the header). 
...
On each item line, therefore, the user can freely (manually) add these additional data, and the result will be that the item line will have this additional section: 

```xml
    <RiferimentoAmministrazione>RifAmm</RiferimentoAmministrazione> 
```

Where the value ‘RifAmm’ is taken from the “Description” of the extra data entered in the item line. 
...
In the **IdDocumento** tag, the text present in the **Your Reference** field of the **customer order header** linked to the invoice item line is reported, and **in the absence of such a value**, the *Your Reference* of the **invoice header** is reported. 

This tag is inserted only for invoice lines that are linked to customer order lines. 

If the invoice has not been created from customer orders in Fluentis to have the reference to the purchase order, a code must be entered in the **Order Ref** field present for each line of the invoice in the detail tab for line **Item data**.
...
- In the **[Types of expenses](/docs/configurations/tables/general-settings/expenses-types)** table, the expense of type “stamp duty” must be entered with a flag on the *Stamp* field and with the *Stamp
value* of 2 euros. 
- In the customer registry, a stamp duty expense must be added in the **[Expenses Discounts](/docs/erp-home/registers/contacts/create-new-contact/accounting-data/customer-vendors-data/charges-discounts)** section, selecting the code previously created in the *Types of expenses* table. 
- To not charge it to the customer: in the stamp duty expense in the customer registry, the value must be 0. 
- In the **[Company)](/docs/configurations/tables/general-settings/company)** table, fill in the *Stamp charges* field (2 euros) and *Min roof expenses* (77.47 euros). 
...
In the purchase order data tag, the CIG and CUP are contained; to report these 2 codes, the invoice must refer to a customer order in Fluentis or the order reference must be specified in the corresponding edit present in the item Data tab of the invoice. 
...
- item quantity: decimals from 2 to 8 
...
- In the **Withholding Types** table, correctly code the code for electronic invoicing (RT01, RT02). 
...
- Create the XML file that reports the indication of the withholding and, for each line, the tag "Withholding YES" if the line is subject to withholding. 
...
PLEASE NOTE THAT IN THE CONFIGURATION OF THE ASSOCIATED WITHHOLDING TYPE, THE **Mat. val. types** field must be set to *Net maturity values*, otherwise the accounting entry and the open item calculation will result incorrect and lacking the transfer of the withholding.
:::
