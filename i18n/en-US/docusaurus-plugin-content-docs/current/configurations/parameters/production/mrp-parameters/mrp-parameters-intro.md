...
<details>
<summary> The following fields are used only in the procedure of [F.C.S Scheduling](/docs/planning/ms-master-scheduling/fcs-scheduling) </summary>

**Pick quantity**: Withdrawals from the warehouse occur only when a quantity at least equal to this value is available (unless the quantity to withdraw is less than the pickup lot). For example, if the value is 50, withdrawals are only made if there are at least 50 pieces in stock or if the quantity to be withdrawn is less than 50. If the value is 0, there is no overlap between the order providing the item and the order requesting it (withdrawals are made only when the entire requested quantity is available in stock). In practice, the value 0 is used if overlap should be prohibited (delivery times increase, but the probability of missed appointments between the order requesting the item and the one providing it is lower, resulting in a production plan that is more likely to be executed without changes), while the value 1 is used to maximize overlap (the best possible deliveries are obtained, but the likelihood that the plan is executed as simulated decreases, increasing the probability of less use of production resources).

**Stock quantity**: A warehouse transfer is made only when a quantity greater than or equal to this value is available (unless the remaining quantity to be stored is less). If the value of this field is 0, the order that provides this item will transfer its quantity to the warehouse at once; partial transfers are not allowed. For purchase and subcontracting orders, this value is considered as if it were 0, despite its actual value. 
For example, consider a production order for 100 pieces. If the transfer lot is 0, a single transfer of 100 pieces to the warehouse will be made when the order reaches 100% completion, meaning those 100 pieces will all be available only after the order completion (no overlap of production orders). 
For example, if the inventory lot is 25, no transfer to the warehouse will occur until the produced quantity not transferred reaches 25. If the order quantity were 60, we would have two transfers of 25 and one transfer of 10 (partial overlap of orders). If the transfer lot is equal to 1, there will be maximum possible overlap, as each produced part will be immediately transferred to the warehouse and will be available for orders requesting this item (this makes sense if the production quantity is small; otherwise, we would have many warehouse transfers).
</details>
