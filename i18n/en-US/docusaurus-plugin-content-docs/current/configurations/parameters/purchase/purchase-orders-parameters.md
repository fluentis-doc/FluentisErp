...
**Automatic proposal for alternative UM (Proposta automatica UM alternativa):** if activated, it ensures that the quantity related to the alternative unit of measure is proposed, provided that a default one has been set on the item registry. This flag can only be activated if the previous flag has been activated.
...
**Block document entry on holidays (Blocca inserimento documento in date festive):** if active, the system does not allow the entry of the order on holidays (Saturday, Sunday, and holidays). If not active, the system does not perform any checks and allows the order to be entered.

**Propose brand with priority supplier list/customer order (Proponi marca con priorità listino fornitore/ordine cliente):** no longer used.

**Allow VAT modification (Permetti modifica IVA):** if active, in the case of [Automatic Order Creation from Purchase Requests](/docs/purchase/purchase-orders/procedures/create-purchase-orders-from-purchase-requests), the VAT code is proposed from the supplier registry.

**Operator code mandatory (Codice operatore obbligatorio):** if active, it is necessary to specify the operator code in the header, under the *Country* entry, otherwise it will not be possible to save the order. When the flag is not active, this data is optional.
...
**Packings accounting**: allows the accounting of rows with packaging items, which are usually managed for verifying the stock of [Returnable Packaging (Imballi a rendere)](/docs/configurations/tables/logistics/package-to-be-returned).

### Load

In this tab, the parameters used for [Goods Receipt Load (Carico ricevimento merci)](/docs/purchase/goods-reception/procedures/good-receipt-load) are defined.
...
*Default values (Valori di default)*: the CDC or CDP is retrieved from the invoiced type if present. For further details, refer to the table [Purchase Invoiced Type (Tipo Fatturato Acquisti)](/docs/configurations/tables/purchase/purchase-invoices-type). If it is not present in the invoiced type, the system will look for it in *Supplier Registry (Anagrafica fornitore)*. If not present, it will be searched in *Item registry* and subsequently at the *Warehouse* level.

**Recompute**: if active, this flag recalculates the cost/profit centers according to the chosen priority.

**Mandatory cost/revenue center (Centro di costo/ricavo obbligatori)**: if active, this flag makes the entry of cost/profit centers mandatory.
