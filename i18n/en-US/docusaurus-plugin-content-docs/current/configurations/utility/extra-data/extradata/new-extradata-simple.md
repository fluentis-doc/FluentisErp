---
title: Simple ExtraData 
sidebar_position: 1
---

## Simple ExtraData - Introductory Aspects 
...
In this case, we proceed to create a **simple** ExtraData of type string called **Model**.

![](/img/en-US/extradata/20250313151723.png)

The detail form for creating ExtraData is composed as follows:

* **Code**: which identifies the extra data.
* **Content type**: will be of a simple type.
...
* **Name**: must be a word present in the dictionary; otherwise, we will need to add it.  
* **Data type**: this is the type of data.
* **Stereotype**: additional options on the field formatting, such as multiline text or defining the number of decimals.
* **Description**: is mandatory - description that will be displayed, just like the Name can be selected from the dictionary.
* **Validity Start/End**: if we want to assign temporary validity to the extra data.
* **View right / Modify right**: to associate a specific right that allows the viewing or modification of the extra data.
...
* **Property**: this is the property of the parent object where the ExtraData will be loaded. In this case, the ExtraData property is present on the FSItem object, generated by the reference that links FSItem to FSItemExtraData.  
* **Mandatory**: indicates whether the ExtraData is mandatory for the associated object.
* **To Print**: indicates whether the ExtraData should be available for printing.
* **Ordering**: this refers to the ordering of the parent object; it does not have repercussions on functionality and is usually left at 0.
...
* **Stereotype**: the formatting of the field, which changes depending on the type of data. It would override the general stereotype defined for the extra data if one wanted to enforce a specific behavior for a particular object different from the default one.
...
Finally, also in the detail form of the item, there is a specific tab called **Extra data** where we can find each ExtraData associated with the item.  
