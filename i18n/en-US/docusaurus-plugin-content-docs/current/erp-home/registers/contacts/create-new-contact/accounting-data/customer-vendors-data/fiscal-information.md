...
**Exclude from black list declaration (Escludi da dich. black list)**: obsolete field present for backward compatibility. The tax compliance requirement is currently abrogated.
...
**Office Code / Recipient Code (Codice Ufficio / Codice destinatario)**: *IPA* code of the public office recipient of the invoice or (if the *Electronic invoicing* flag is active) *SDI* code of the invoice recipient.

**Office Name / Recipient Description (Nome ufficio / Descrizione destinatario)**: description of the office code / recipient code field to be filled in with the name of the public office or private entity.
...
> **Sol. inf. 30 gg (Soluzione inferiore a 30 giorni)**: imposes a payment solution at 30 days bypassing other settings (including those related to art. 62, for example, the solution at 60 days).  
> **Sol. inf. 60 gg**: NOT MANAGED / DEACTIVATED

There are other settings regarding the **management of perishable goods**:  
> **GG Pag. merce deperibile (Giorni pagamento merce deperibile)** within the [Item Registry > General tab](/docs/erp-home/registers/items/create-new-items/item-registry/generality) where the expiration to be applied is defined;  
...
**Client/mandate code (Codice cliente/mandato)**: in this field, the identification code assigned to the mandate signed by the debtor must be entered. It corresponds in the Rulebook (official technical document) to AT-01 Unique Mandate Reference.
...
**First execution date / Last execution date (Data prima esecuzione / Data ultima esecuzione)**: since the SDD (like the "old" RID system) is a periodic execution by nature, this date range checks the beginning and end of the execution's validity.
...
- **Creditor identifier (Identificatore creditore)**: example IT95ZZZ0000007333890900
