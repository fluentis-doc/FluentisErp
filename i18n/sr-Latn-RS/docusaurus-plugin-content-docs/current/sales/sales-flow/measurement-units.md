...
Ova funkcionalnost omogućava definisanje i korištenje različitih jedinica mjere za isti proizvod u različitim kontekstima, kao što su prodaja, nabava ili upravljanje skladištem.  
...
Opcije prisutni u tabu Alternativne jedinice mjere kartona artikla utječu na prijedlog i izračun količina u dokumentima. Ako je aktiviran flag Zadano, ta jedinica mjere će se koristiti za automatski prijedlog. Ako je aktiviran flag Obavezno, svaka promjena glavne ili alternativne količine rezultirat će automatskim preračunom odgovarajuće količine, zadržavajući vezu preko faktora konverzije. S aktiviranim flagom Prioritetna, alternativna količina ima prednost, a glavna količina se izračunava bez ponovnog preračuna alternative.  

Izračun alternativne količine temelji se na faktoru konverzije, koji se može množiti ili dijeliti s glavnom količinom; taj izbor se konfigurisa u [Početnim parametrima skladišta](/docs/configurations/parameters/logistics/warehouse-initial-parameters/warehouse-parameters) putem odgovarajućih opcija.  
