...
L'insertion d'une commande de vente demande le type commande prédéfini en [Home  >  Tableaux > Ventes]. Avec l'insertion du client viennent proposés en automatique tous les données spécifiés du tab 'En-tête', selon les données établies précédemment dans le registre client. Les données proposées, si spécifiés dans le registre, sont:
...
**Catalogues**: il vient proposée l'information insérée dans le registre, tab 'Catalogues', mais seul s'existe un offre par défaut (tableau de référence 'Catalogue de vente');
...
**Notes initials**: il est possible d'insérer des notes qui on été inséré precédémment en 'Notes codées', et donc, l'utilisateur doit effectuer double click sur le champ 'Note initial' pour ouvrir 'Aide note codées' et pour sélectionner les données, ou avec le touche droit du souris est possible d'ouvrir une fenêtre pour pouvoir spécifies un long text de note, si le champ contient une valeur viendra changé le couleur du fond du champ;

**Annotations client**: il vient proposée l'information insérée dans le registre client, tab 'Administratif'; 

**État commande**: la commande, combien vient créée, a l'état 'N'exécutée pas'. Mais pour transferir les articles par la commande dans l'ordre, la commande doit avoir le flag 'Imprimée' e la date confirmation commande. Après les opérations de transfert, la commande change l'état en automatique.
...
| --- | --- |
| Enregistrer | Bouton pour enregistrer les commandes de vente. |
| Supprimer les paiements | Il permet de supprimer les paiements insérés dans l'en-tête; les paiements sont reprise dans le registre. |
