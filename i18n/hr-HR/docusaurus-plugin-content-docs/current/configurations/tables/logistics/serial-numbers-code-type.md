...
La tabella permette di impostare i tipi di codici serial number da gestire poi nel database.
...
Il **Tipo codice serial number** deve poi essere agganciato all'*Anagrafica articolo* nel campo omonimo situato nel tab *Lotti e serial number*, in modo che nelle procedure che creano automaticamente il codice serial number per l'articolo in questione il SN stesso venga generato secondo le regole impostate qui.
