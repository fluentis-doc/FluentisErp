...
Per quanto riguarda la modifica registrazioni di magazzino da documenti teniamo presente che è possibile scegliere se modificare la quantità, modificare il prezzo, modificare entrambe o nessuna. Questo significa che quando un documento già registrato a magazzino, per esempio una DDT di acquisto, viene modificato, l'applicazione controlla se il flag Modifica quantità e il flag Modifica prezzi sono attivi oppure no; in caso affermativo la modifica eseguita nel documento viene trasmessa anche nella registrazione di magazzino con cui il documento è collegato, diversamente la modifica del documento non interviene sulla relativa registrazione di magazzino. È possibile attivare anche uno solo dei due flag.
...
**Obbligo magazzino per articoli gestiti a lotti**: impone all'utente di inserire, nei documenti di scarico di articoli gestiti a lotti, il magazzino e la causale di scarico prima di selezionare il lotto da scaricare. Diversamente l'utente può decidere di scaricare, sulla stessa riga articolo del documento, un lotto da un magazzino e un altro lotto da un altro magazzino, non avendo più la necessità di indicare da quale magazzino verranno scaricati i lotti a livello di riga documento. 

**Gestione stato lotto tramite Workflow**: 	

**Usa barcode movimenti**:  


## Parametri chiusura

In questo tab è possibile decidere per quali **Magazzini** devono essere eseguite le chiusure per **Variante**, per **Ubicazione**, per **Lotti**, per **Commesse di vendita** e per **Cliente/fornitore**. In pratica attivando uno di questi flag si decide che la registrazione di carico per inventario iniziale eseguita dalla procedura automatica di *Valorizzazione magazzino* deve essere creata tenendo conto dell'impostazione eseguita.
...
*Esempio*
Nel magazzino *Conto lavoro* solitamente si tiene attivato il flag sulla colonna *Cliente/Fornitore*, in quanto è utile che la procedura crei una registrazione di magazzino di carico per inventario iniziale sul magazzino conto lavoro per ciascun terzista; nei magazzini in cui vengono stoccate e movimentate merci gestite a lotti si consiglia di attivare il flag *Lotti/SN*, così come sui magazzini in cui vengono stoccate e movimentati merci gestite a varianti si consiglia di attivare il relativo flag.


## Parametri giacenza

In questo tab è possibile decidere per quali magazzini si possa gestire la *Giacenza negativa*. 

**Calcolo giacenza per articolo, magazzino**: le giacenze di magazzino e quindi la visualizzazione giacenze possono essere viste sulla base dei parametri qui definiti; quindi si può ottenere un calcolo preciso delle giacenze per *Variante*, per *Ubicazione*, per *Lotto*, per *Commessa* o per *Cliente/fornitore* a seconda dei flag attivi.
...
**Giacenza commessa**: significa controllare la giacenza anche per la commessa di vendita (progetto) specificata nel movimento (se la giacenza per quella commessa non è sufficiente, il movimento non è creato); 
...
**Duplica articolo in altra società**: questo flag consente di decidere in quali altre società possono essere duplicati gli articoli utilizzando la procedura *Duplica Articoli*.