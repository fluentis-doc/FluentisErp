...
Ovaj postupak koristi se za popunjavanje inventurne liste koja je kreirana u *wpf* s artiklima koji se čitaju pomoću barkoda. Ako se artikl pročita drugi put (isti Klasa/Kod/Lot (ako se upravlja po lotovima)) i navede na istoj lokaciji kao i prvi put, pojavit će se skočni prozor koji pita želite li **Dodati** ili **Zamijeniti** količinu koja je prethodno pročitana.
...
Za sve informacije o tome kako kodirati barkodove za čitanje u polju **šifra Barkoda** pogledajte stranicu koja se odnosi na [Barkod tokenizator](/docs/configurations/tables/general-settings/barcode-tokenizer).
