---
title: Form Navigator
sidebar_position: 1
---

**Form Navigator** vă permite să personalizați form-urile cu ajutorul profilelor utilizatorilor.

Problematiche correlate:

 [Adăugare câmp cu Form Navigator și Object Navigator](/docs/object-navigator/object-navigator-intro) 

 [Setări date în griduri](/docs/form-navigator/data-grid-settings) 

 [Personalizări form](/docs/guide/common/operations-with-data/form-customization-and-profiles-management) 

 [Personalizări cu ajutorul Form Editorului](/docs/form-navigator/form-editor) 






