---
title: Plăți
sidebar_position: 1
---

Soluțiile de plată sunt preluate automat din registrul terțului și pot fi modificate/șterse de utilizator.

RIBBON BAR: reprezintă meniul cu butoanele de executie, pentru fiecare operatie in parte aferentă fiecarei ferestre din aria in care se lucreaza. Lista functionalitatiilor posibile conține:



| Functie | Functionalitate |
| --- | --- |
| Șterge plăți | Buton pentru ștergerea plăților selectate în gridul plăților. |






