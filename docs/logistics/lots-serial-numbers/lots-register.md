...
sidebar_position: 3
sidebar_label: "Anagrafica lotti e Tracciabilità"
description: "Guida all'uso dell'anagrafica lotti e del modulo di tracciabilità in Fluentis per una gestione efficiente dei prodotti."
schema: "TechArticle"
tags: ["anagrafica lotti", "tracciabilità", "Fluentis", "gestione prodotti"]
keywords: ["anagrafica lotti", "tracciabilità prodotti", "Fluentis"]
---

# Anagrafica lotti e Tracciabilità
...
Un aspetto distintivo dell'anagrafica lotti è la sezione che consente di visualizzare storicamente i documenti di carico e scarico che hanno coinvolto il lotto, mantenendo così una tracciabilità accurata nel tempo. È possibile visualizzare la giacenza attuale suddivisa per magazzino. Grazie a queste funzionalità, l'anagrafica lotti di Fluentis semplifica la gestione operativa e contribuisce a garantire la compliance normativa in settori dove la tracciabilità è essenziale, come alimentare e farmaceutico.
:::
...
- **Articolo**: classe, codice e descrizione dell'articolo a cui è riferito il lotto. Si tratta di un articolo con il flag *Gestione lotti* attivato nell'*Anagrafica articolo* > tab *Lotti/SN*, altrimenti non sarebbe stato possibile eseguire il carico del lotto per l'articolo stesso.
- **Variante**: codice e descrizione della variante dell'articolo a cui è riferito il lotto, se presente.
...
- **Fornitore**: conto/sottoconto/descrizione del fornitore.
- **Produttore**: nomignolo/ragione sociale del produttore.
- **Codice fornitore**: codice del *Lotto fornitore*, quindi di solito il nome che il fornitore attribuisce al lotto. Si tratta di un campo editabile da parte dell'utente e non generabile automaticamente durante le operazioni di carico. Anch'esso può raggiungere la lunghezza di 50 caratteri come limite massimo e può essere identico anche per lotti diversi di articoli diversi.
- **Tipo report etichetta**: tipo di etichetta da stampare per questo lotto.
- **Data inizio**: data in cui è stato caricato il lotto, editabile dall'utente. Se il lotto è stato caricato tramite la registrazione di magazzino, questa data viene proposta identica alla data in cui è stata creata la registrazione; se il lotto è stato caricato tramite carico di una DDT di acquisto o registrazione rientro di conto lavoro, questa data viene impostata come uguale alla data del DDT di acquisto o di conto lavoro (a meno che all'interno del DDT l'utente non l'abbia modificata manualmente); se il lotto è stato caricato tramite registrazione di una segnalazione di produzione, questa data viene proposta come identica alla data della segnalazione di produzione.
- **Data scadenza**: data di scadenza del lotto, calcolata al momento della creazione e quindi di carico del relativo movimento di magazzino, partendo dalla data di inizio e sommando i giorni di validità del lotto inseriti nel tab *Lotti/SerialNumber* dell'anagrafica articolo.
- **Quantità iniziale**: quantità iniziale caricata per il lotto tramite il primo documento che ha caricato il lotto stesso.
- **Numero colli**: numero di colli del lotto caricato.
- **Note**: note inserite nella registrazione di magazzino contestualmente al carico del lotto. Il campo è editabile dall'utente e visualizzabile anche nella Ricerca Lotti, come sopra specificato.
- **Stato lotto**: stato del lotto, modificabile e proveniente dall'apposita tabella *Stati lotto*.
- **Valutazione lotto**: dato relativo alla valutazione del lotto, facoltativo. Lo stato del lotto determina la possibilità di utilizzare o meno il lotto nei documenti di vendita, di produzione e nelle registrazioni di scarico di magazzino. Si tratta comunque di una gestione non vincolante, l'utente non è quindi obbligato a sfruttare la funzionalità della tabella *Stati lotto*.
...
- **Stato Lotto**: stato del lotto.
- **Chiuso forzatamente**: flag per indicare la chiusura forzata del lotto.
- **Documento di riferimento**: tipo di documento che ha generato il carico.
- **Numero**: numero del documento.
- **Data inserimento**: data del documento.
- **Magazzino**: codice e descrizione del magazzino su cui è stato caricato il lotto.
- **Ubicazione**: codice e descrizione dell'ubicazione.
- **Causale**: codice e descrizione della causale utilizzata per la creazione del movimento di carico.
- **Cliente/Fornitore**: conto, sottoconto e descrizione relative al cliente o fornitore.
- **Progetto**: commessa di vendita legata al lotto, editabile.
...
- **Ubicazione**: codice e descrizione dell'ubicazione.
- **Causale**: codice e descrizione della causale.
- **Cliente/Fornitore**: conto, sottoconto e descrizione relativa al cliente o fornitore.
- **Progetto**: commessa di vendita, editabile dall'utente, legata al lotto. Di solito si tratta della commessa di vendita agganciata alla riga dell'articolo che è stata caricata e che ha caricato il lotto stesso.
...
**Tipo documento**: tipo all'interno del quale è presente il lotto, in relazione al documento selezionato nella griglia superiore. Esso può essere una DDT di vendita, una fattura di vendita, una lista di prelievo, una DDT di consegna di conto lavoro, un rientro di conto lavoro.
...
- **Codice**: codice del magazzino.
- **Descrizione**: descrizione del magazzino.
- **Quantità totale caricata**: quantità totale caricata di quell'articolo per quel lotto.
- **Quantità giacenza**: giacenza attuale di quell'articolo per quel lotto.
- **Quantità disponibile**: giacenza disponibile di quell'articolo per quel lotto.
...
- **Tracciabilità Impiego**: visualizza gli articoli nei quali è stato utilizzato il lotto, garantendo così una verifica dettagliata della tracciabilità verso l'alto.
- **Tracciabilità Composizione**: verifica quali articoli gestiti a lotti sono stati impiegati per la produzione del lotto selezionato, fornendo una chiara visione della tracciabilità verso il basso.

Attraverso questa funzionalità, Fluentis offre una soluzione robusta per le aziende, specialmente in settori come quello alimentare e farmaceutico, dove la rintracciabilità è fondamentale per la sicurezza e la compliance alle normative vigenti.
:::

Questa form permette di stampare un report contenente il dettaglio della tracciabilità del lotto selezionato. 
Dopo aver impostato i filtri desiderati, cliccare sul pulsante **Ricerca** per visualizzare i risultati.
...
L'anagrafica lotti e la tracciabilità in Fluentis sono strumenti fondamentali per la gestione e il controllo dei lotti di prodotto. Permettono di monitorare ogni passaggio del lotto, dalla sua creazione fino al suo utilizzo finale. Grazie a funzionalità mirate, le aziende possono garantire conformità normativa e gestione efficace dei materiali. 

Per ulteriori dettagli, si rimanda a sezioni correlate nel documento.