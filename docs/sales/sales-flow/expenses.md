...
All’interno dell’anagrafica cliente è possibile definire gli oneri applicabili. Per ogni riga spesa sono presenti quattro flag che determinano a partire da quale fase del ciclo documentale l’onere deve essere ricalcolato:
...
4. Proposta e propagazione degli oneri

Quando un nuovo documento viene creato, il sistema propone automaticamente gli oneri definiti in anagrafica cliente.

Nel caso in cui il documento venga generato a partire da un documento precedente tramite evasione o valorizzazione, gli oneri presenti nel documento sorgente vengono trasferiti nel documento di destinazione secondo regole precise.
...
5. Comportamento con flag di calcolo attivo
...
6. Gestione del minimo imponibile

La gestione del minimo imponibile sugli oneri percentuali rimane invariata rispetto alla logica attuale.

Quando un onere è definito in percentuale e presenta un valore minimo associato, il sistema verifica il risultato del calcolo. Se l’importo risultante è inferiore al minimo configurato, viene applicato il valore minimo previsto.

Nel caso in cui l’onere venga trasferito come importo fisso in un documento di destinazione, il meccanismo di ricalcolo del minimo non è più applicabile. In questo scenario, i minimi provenienti da documenti diversi vengono semplicemente sommati senza ulteriori ricalcoli.

7. Modifiche ai documenti dopo evasione o valorizzazione
...
Poiché non è possibile determinare automaticamente un nuovo ricalcolo corretto, il sistema deve notificare l’utente con un messaggio informativo che segnala la necessità di intervento manuale.
...
8. Nota funzionale per l’utente

È importante sottolineare che il calcolo del minimo onere si applica esclusivamente agli oneri di tipo percentuale.

Quando un onere viene trasformato in importo fisso durante un processo di evasione, il concetto di minimo non può più essere ricalcolato in modo dinamico. Per questo motivo, in presenza di più documenti sorgente, gli eventuali minimi vengono trattati come valori già consolidati e sommati tra loro senza ulteriori verifiche.

Nel caso in cui sia necessario mantenere una coerenza rigorosa del minimo, è obbligatorio utilizzare i flag di calcolo a livello di ciclo documentale, così da forzare il ricalcolo dell’onere nel documento di destinazione.