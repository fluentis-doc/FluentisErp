...
> **Descrizione**: è la descrizione che verrà utilizzata per la riga di *Articolo spesa* della nuova *Nota di accredito* (dato obbligatorio); viene proposta la descrizione che viene parametrizzata nei [Parametri di Valorizzazione reclami](/docs/quality/claims-and-non-compliance/non-compliances/procedure), sessione *Riferimenti reclami*.   
