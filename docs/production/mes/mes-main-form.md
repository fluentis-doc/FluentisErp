...
description: Guida completa alla form MES di Fluentis per la dichiarazione delle attività produttive e il monitoraggio OEE.
keywords:
  - MES
...
La form principale del Modulo di Esecuzione della Produzione (MES) di Fluentis è progettata per ottimizzare e semplificare il processo di dichiarazione delle attività produttive.

Consente agli operatori di:

- Monitorare in tempo reale le risorse di produzione.
- Inserire e modificare dati relativi a fasi di lavorazione e quantità prodotte.
- Utilizzare il **Barcode Tokenizer** per velocizzare l’inserimento dati.
- Applicare controlli specifici per garantire la correttezza delle registrazioni.
- Ripartire tempi e quantità secondo diverse logiche di analisi.

L’approccio integrato migliora la visibilità operativa, ottimizza i flussi di lavoro e incrementa la produttività aziendale.
:::

### Pulsanti specifici

#### Navigabilità  
- **Precedente**: passa al tab precedente.  
- **Successivo**: passa al tab successivo.  
- **Aggiorna**: aggiorna tutti i dati della form (utile dopo modifiche alla risorsa).  
- **Cancella selezione**: annulla la selezione delle fasi nella griglia principale.  

#### Conferma segnalazione  
- **Inizia**: crea una segnalazione di apertura.  
- **Avanza**: crea una segnalazione di avanzamento.  
- **Sospendi**: crea una segnalazione di sospensione.  
- **Riprendi**: crea una segnalazione di ripresa.  
- **Chiudi**: crea una segnalazione di chiusura.  
- **Annulla**: azzera i dati inseriti nel MES.
...
La configurazione avviene nella sezione: `Tabelle - Impostazioni generali - Barcode tokenizer`
Quindi, il campo *Codice barcode*, nel MES, può essere utilizzato dall’operatore per aggiungere singole fasi o gruppi di fasi, o ad esempio anche le fasi di un intero ordine di produzione, oppure ancora caricare i lotti e/o serial number dei materiali da scaricare, il tutto semplicemente leggendo i vari barcode.  
Per informazioni su come codificare i barcode da leggere nel campo **Codice barcode** consultare la pagina: [Barcode tokenizer](/docs/configurations/tables/general-settings/barcode-tokenizer)
...
Approfondimento: [Workstation](/docs/production/mes/workstation)
...
Permette la gestione dei controlli qualità sugli articoli prodotti o scartati.
...
**Rilevazione valori multipli**  
La tabella viene utilizzata nel caso in cui siano rilevati più valori e questi concorreranno a determinare la *Media valori rilevati*.
...
È possibile modificare il funzionamento e il layout della form tramite la gestione delle [Risorse di produzione](/docs/production/mes/production-resources). 
Approfondimenti sull'area relativa alla gestione della: [Qualità](/docs/quality/quality-intro)
...
Il tab è dedicato alla visualizzazione e consultazione dei dati relativi alle performance dell'attività di produzione; questo si compone di tre sezioni:  
- Diagrammi OEE
...
Il **MES di Fluentis** consente la gestione completa delle dichiarazioni di produzione, integrando:

- Inserimento tempi e quantità.
- Gestione lotti e serial number.
- Controllo qualità.
- Analisi KPI tramite OEE.
- Gestione squadra e documentazione.

La configurazione avanzata del comportamento della form è centralizzata nelle:
...
- [Qualità](/docs/quality/quality-intro)

Questa integrazione garantisce tracciabilità, controllo delle performance e ottimizzazione dei processi produttivi in ambiente ERP.