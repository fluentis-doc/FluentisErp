...
Il tipo **Sovrapposizione a percentuale**, attivo solo per la [Schedulazione F.C.S](/docs/planning/ms-master-scheduling/fcs-scheduling), viene usato quando la quantità da produrre è molto piccola, (1 o poche unità) e le ore di lavorazione non sono poche (se fossero poche si potrebbe evitare di indicare la sovrapposizione). In tal caso, definire la sovrapposizione tramite la quantità, può essere impossibile (quantità pari ad 1) o assai poco preciso. Se si usa la percentuale di sovrapposizione, la fase può essere schedulata solo se quella che la precede ha raggiunto una percentuale di avanzamento pari al valore indicato come percentuale di sovrapposizione, ma non si deve usare un valore pari a zero poiché verrebbe inteso come assenza di valore e quindi sovrapposizione non ammessa. Se per esempio, si vuole che la fase inizi appena possibile dopo l’inizio di quella che la precede, indicare un valore molto piccolo maggiore di zero.
