...
**Creazione ordini pianificati dal calcolo MRP**: se attivo, questo flag fa sì che l'MRP crei gli ordini pianificati (sempre se attivo con l'omonimo flag presente nel tab Parametri dell'[MRP](/docs/configurations/parameters/production/mrp-parameters/mrp-parameters-intro)); mentre se non attivo, l'MRP creerà solamente la proposta degli ordini pianificati (senza creali).
