...
declare @filePattern nvarchar(100) = 'DocFinCashFlow.txt' -- [BZFD_FileNamePattern] - used in flow document (Example: N'^(f|F)attura_([a-z]|[A-Z]|[0-9]|_)+.(x|X)(m|M)(l|L)')
...
--Flow operation
declare @flowOpName nvarchar(100) = 'FLOW_OP_StdCashFlowExport' -- Flow operation name (Example: N'FLOW_OP_StdAccountDataImportFromExcel')
declare @flowOpDesc nvarchar(2048) = 'Esportazione CashFlow' -- Flow operation description (Example: N'Standard account data import flow operation from Excel file')
declare @flowOpIsImport bit = 0 -- = 0 = Export, 1 = Import
declare @flowOpExportWorkflow nvarchar(1024) = 'class:Fluentis.BizLink.Core.WorkFlows.ExportWorkflows.DocFinCashFlowExportWorkflow, Fluentis.BizLink.Core.DocFin' -- Export workflow. If it's an import it is usually NULL

--Flow document
declare @flowDocName nvarchar(100) = 'FLOW_DOC_StdCashFlowExport' -- Flow document name (Example: N'FLOW_DOC_StdAccountDataImportFromExcel')
declare @flowDocDesc nvarchar(2048) = 'Esportazione CashFlow' -- Flow document decription (Example: N'Standard account data import flow doc from Excel file')

-- Connector
declare @connName nvarchar(100) = 'CONN_StdExportCashFlow' -- Connector name (Example: N'CONN_StdExcelAccountDataImport')
declare @connDesc nvarchar(2048) = 'Export CashFlow for doc finance' -- Connector description (Example: N'Connector for importing account data from Excel file')
...
declare @parserObjName nvarchar(100) = 'PO_StdFluentisDocFinanceExportCashFlow' -- Parser object name (Example: N'PO_StdFluentisAccountDataImportFromExcel')
declare @parserObjDesc nvarchar(2048) = 'Export CashFlow for doc finance' -- Parser object description (Example: N'Parser object for account data import')
declare @parserWorkflow nvarchar(2048) = NULL -- usually NULL.  (Example: NULL)

-- Configuration (XML of the parser object)
declare @parserObjConfig nvarchar(max) = '<Object name="CashFlow" alias="CashFlow" description="CashFlow">
  <FluentisExportObject name="CashFlow" type="Fluentis.FluentisErp.Core.PR.CashFlow.FSCashFlowMovement, Fluentis.FluentisErp.Core.PR.CashFlow" default="true" />
...
-- BizLink object
declare @bzObjName nvarchar(100) = 'BO_StdFluentisDocFinCashFlowExport' -- Name of the object (Example: N'BO_StdFluentisAccountDataImportFromExcel')
declare @bzObjVer nvarchar(100) = '1.0' -- Version (Example: N'1.0')
declare @bzObjDesc nvarchar(max) = 'Export doc finance CashFlow' -- Description (Example: N'Standard import of account data from Excel file')
declare @bzObjAlias nvarchar(100) = 'BO_StdFluentisDocFinCashFlowExport' -- Alias (Example: N'BO_StdFluentisAccountDataImportFromExcel')
...
declare @bzObjConfig nvarchar(max) = '<Object name="CashFlow" alias="CashFlow" description="CashFlow" />' -- BizLink Object XML. Enter the configuration XML for the BizLink Object. Use a nvarchar string: N'....'
-- **************************************************************************************************
