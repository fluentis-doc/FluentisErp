...
declare @filePattern nvarchar(100) = 'DocFinAnagrafiche.txt' -- [BZFD_FileNamePattern] - used in flow document (Example: N'^(f|F)attura_([a-z]|[A-Z]|[0-9]|_)+.(x|X)(m|M)(l|L)')
declare @parserName nvarchar(100) = 'Txt' -- Name of the parser to use (Example: N'Xml'). See BZ_Parsers
declare @connTypeName nvarchar(100) = 'Folder Output' -- Name of the connector type (Example: N'Folder Monitor'). See BZ_Connectors
...
--Flow operation
declare @flowOpName nvarchar(100) = 'FLOW_OP_StdAccountExport' -- Flow operation name (Example: N'FLOW_OP_StdAccountDataImportFromExcel')
declare @flowOpDesc nvarchar(2048) = 'Esportazione anagrafiche' -- Flow operation description (Example: N'Standard account data import flow operation from Excel file')
declare @flowOpIsImport bit = 0 -- = 0 = Export, 1 = Import
declare @flowOpExportWorkflow nvarchar(1024) = 'class:Fluentis.BizLink.Core.WorkFlows.ExportWorkflows.DocFinAccountExportWorkflow, Fluentis.BizLink.Core.DocFin' -- Export workflow. If it's an import it is usually NULL

--Flow document
declare @flowDocName nvarchar(100) = 'FLOW_DOC_StdAccountExport' -- Flow document name (Example: N'FLOW_DOC_StdAccountDataImportFromExcel')
declare @flowDocDesc nvarchar(2048) = 'Esportazione anagrafiche' -- Flow document decription (Example: N'Standard account data import flow doc from Excel file')

-- Connector
declare @connName nvarchar(100) = 'CONN_StdExportAccount' -- Connector name (Example: N'CONN_StdExcelAccountDataImport')
declare @connDesc nvarchar(2048) = 'Export account for doc finance' -- Connector description (Example: N'Connector for importing account data from Excel file')
declare @connConfig nvarchar(max) = '<Configuration outgoingFolder="C:\Temp" append="false" textEncoding="UnicodeText" />' -- Configuration for the connector (Example: N'<Configuration fileMask="*.*" incomingFolder="C:\BizLink\Input\Incoming" processedFolder="C:\BizLink\Input\Processed" failedFolder="C:\BizLink\Input\Failed" binaryFile="true" />')

-- Parser object
declare @parserObjVer nvarchar(100) = '1.0' -- Parser object version (Example: N'1.0')
declare @parserObjName nvarchar(100) = 'PO_StdFluentisDocFinanceExportAccount' -- Parser object name (Example: N'PO_StdFluentisAccountDataImportFromExcel')
declare @parserObjDesc nvarchar(2048) = 'Export account for doc finance' -- Parser object description (Example: N'Parser object for account data import')
declare @parserWorkflow nvarchar(2048) = NULL -- usually NULL.  (Example: NULL)

-- Configuration (XML of the parser object)
declare @parserObjConfig nvarchar(max) = '<Object name="Anagrafiche" alias="Anagrafiche" description="Anagrafiche">
  <FluentisExportObject name="Anagrafiche" type="Fluentis.FluentisErp.Core.FSCustomerSupplierAccountPaymentsPresentationBank, Fluentis.FluentisErp.Core" default="true" />
...
-- BizLink object
declare @bzObjName nvarchar(100) = 'BO_StdFluentisDocFinAccountExport' -- Name of the object (Example: N'BO_StdFluentisAccountDataImportFromExcel')
declare @bzObjVer nvarchar(100) = '1.0' -- Version (Example: N'1.0')
declare @bzObjDesc nvarchar(max) = 'Export doc finance account' -- Description (Example: N'Standard import of account data from Excel file')
declare @bzObjAlias nvarchar(100) = 'BO_StdFluentisDocFinAccountExport' -- Alias (Example: N'BO_StdFluentisAccountDataImportFromExcel')
declare @bzObjWorkflow nvarchar(2048) = NULL -- Workflow (Example: NULL)
declare @bzObjNoAutoprocess bit = 1 -- No auto process (Example: 0)
declare @bzSkipWorkflow bit = 0 -- Skip Workflow (Example: 0)

-- Configuration (XML of the BizLInk object)
declare @bzObjConfig nvarchar(max) = '<Object name="Anagrafiche" alias="Anagrafiche" description="Anagrafiche" />' -- BizLink Object XML. Enter the configuration XML for the BizLink Object. Use a nvarchar string: N'....'
-- **************************************************************************************************
